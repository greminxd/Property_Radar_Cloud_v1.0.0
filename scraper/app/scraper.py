...
class Scraper:
    ...
    async def collect_source(self,browser,source):
        ...
        diagnostics are used to decide whether a portal scan was healthy. A portal that
        returned zero links or was blocked must never deactivate the previous good data.
        """
        ...
        for search_url in source["search_urls"]:
            ...
            for _ in range(self.cfg["browser"].get("max_search_pages_per_url",4)):
                ...
                try:
                    ...
                    for h in hrefs:
                        ...
                        if pattern.match(h):
                            # First discovery wins. It prevents a recommendation widget on a
                            # garage search from overriding a strong plot discovery and vice versa.
                    ...
                    for sel in ['a[rel="next"]','a:has-text("Następna")','a:has-text("Dalej")','button:has-text("Następna")']:
                        try:
                            ...
                            if await loc.count():
                                ...
                                if href:
                                    nxt=canonical_url(current,href); break
                        except Exception: pass
                    current=nxt
                except Exception as e:
                    errors.append(f"{current}: {type(e).__name__}: {e}"); break
        links=list(link_hints.keys())[:self.cfg["browser"].get("max_detail_pages_per_source",120)]
        results=[]; detail_ok=0; listing_like_count=0
        for u in links:
            try:
                html=await self._goto(page,u,reveal_phone=True)
                detail_ok += 1
                rec=parse_detail(html,u,source["name"],category_hint=link_hints.get(u))
                try:
                    visible=await page.locator("body").inner_text(timeout=1800)
                except Exception:
                    visible=""
                try:
                    tel_hrefs=await page.locator('a[href^="tel:"]').evaluate_all("els => els.map(e => e.href)")
                except Exception:
                    tel_hrefs=[]
                ...
                if listing_like and rec["category"] in self.cfg["filters"]["categories"]:
                    results.append(rec)
            except Exception as e:
                errors.append(f"{u}: {type(e).__name__}: {e}")
            await asyncio.sleep(self.cfg["browser"].get("delay_between_pages_s",0.8))
        await context.close()
        diagnostics={
            "source":source["name"],
            ...
            "records":len(results),
            "errors":len(errors),
            "healthy": bool(search_pages_ok > 0 and len(links) > 0 and detail_ok > 0 and listing_like_count > 0),
        }
