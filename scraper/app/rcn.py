...
RCN_PARSER_VERSION = 'v3.0-ha-paired-price-iso-date-undeveloped'
    ...
    def fetch_recent(self, months=24, radius_km=12, max_features=2500):
        ...
        while start < max_features:
            ...
            for mem in members:
                ...
                # Property-level prices can be repeated for every parcel member. Do not
                # count them multiple times in the benchmark.
                if basis == 'property':
                    key = '|'.join([txid, 'PROPERTY', tx_date, f'{price:.2f}', f'{area:.2f}'])
                else:
